#!/usr/bin/env python3
"""
Trashion API - Railway.app Version
Runs only during business hours (12:00-19:00 Finland time)
Full Selenium scraping with automatic "Load More" clicking
"""
...
def setup_selenium():
    """Setup Chrome for Railway (headless)"""
    options = Options()
...
def scrape_full_inventory():
    """Scrape with automatic Load More clicking"""
    ...
    if not driver:
        ...
        return {"success": False, "error": "Selenium initialization failed"}
    
    try:
        print(f"📡 Loading {TRASHION_URL}")
        driver.get(TRASHION_URL)
        time.sleep(3)
        
        all_ids = set()
        clicks = 0
        max_clicks = 20  # Increased for Railway
        ...
        while clicks < max_clicks:
            ...
            try:
                load_more = driver.find_element(By.CLASS_NAME, 'wpgb-load-more')
                
                # Check if button is visible and enabled
                if not load_more.is_displayed() or not load_more.is_enabled():
                    print("   ✓ Load More button disabled/hidden - all items loaded")
                    break
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("🚂 TRASHION API - RAILWAY VERSION")
    print("="*70)
